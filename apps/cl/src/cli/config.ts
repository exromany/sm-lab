...
async function request(
  ...
  const res = await fetchImpl(`${url}${path}`, init).catch((err) => {
...
export function buildConfigCommand(fetchImpl: typeof fetch = fetch): Command {
  ...
  configCommand
    ...
    .action(
      async (
        ...
        const res = await request(fetchImpl, url, '/admin/validators', {
          method: 'POST',
        ...
        if (!res.ok && res.status !== 207) {
          console.error('Error:', (data.errors ?? [data]).join(', '));
          process.exit(1);
        }
  ...
  configCommand
    ...
    .action(async (_opts, cmd: Command) => {
      const url = resolveUrl(cmd);
      await request(fetchImpl, url, '/admin/validators', { method: 'DELETE' });
  ...
  configCommand
    ...
    .action(async (pubkey: string, _opts, cmd: Command) => {
      const url = resolveUrl(cmd);
      await request(fetchImpl, url, `/admin/validators/${encodeURIComponent(pubkey)}`, {
        method: 'DELETE',
      });
  ...
  configCommand
    ...
    .action((cmdOpts: { json?: boolean }) => {
      if (cmdOpts.json) {
        console.log(JSON.stringify(VALIDATOR_STATUSES, null, 2));
        return;
      }
...
export const configCommand = buildConfigCommand();
