...
**Goal:** Ship the `@csm-lab/recipes` TS API MVP — `connect` + the `actAs` impersonation engine plus five recipes (`addKeys`, `operatorInfo`, `createCuratedOperator`, `setGateAddrs`, `warpBy`/`snapshot`/`revert`) that exercise _every_ seam end-to-end against an anvil fork, with hermetic tests.
...
- **No Foundry.** No `forge`/`cast`/`solc` at build or runtime. `anvil` is the _target_ chain only, reached over RPC via viem.
