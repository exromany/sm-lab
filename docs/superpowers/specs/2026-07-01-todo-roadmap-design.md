...
The repo is renamed to reflect what it _is_ (a staking test bed), not why it was built (CSM work).
...
- **F1 (machine-readable I/O) folded into B, not deferred.** It's a cross-cutting _convention_
...
- **D before/with E** because E realigns onto merkle's reworked surface _and_ both share the IPFS
...
- **Scope codemod:** `@csm-lab/` → `@sm-lab/` across `package.json`, `src/**`, `tsconfig.json`,
  `tsdown.config.ts`, `.env.sample`, docs. (Package `tsconfig` uses _relative_ extends, so those
...
- **Global `.env.sample`:** the root `.env.sample` today only covers `@sm-lab/receipts` refresh.
  Expand it to a single grouped file documenting _every_ env var across all packages (per-package
...
input is a _required file path_; `IPFS_API_URL` unset → **real Pinata**.
...
- **Rewards mode (consolidated from recipes):** merkle already owns `buildRewardsTree`
  (`[nodeOperatorId, cumulativeShares]` leaves). The `rewards` mode owns the _full_ input→tree→pin
...
You chose _machine-readable I/O_ + _explore_. This decomposes into a committed layer and an open
end-state:
...
- **F2 vs F3 — the end-state (open, decided in this phase's own spec):**
  - **F2 — MCP server (`@sm-lab/mcp`)** _(recommended):_ expose recipes / merkle / mock-control as
