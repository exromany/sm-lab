...
Migrate `csm-cl-mock` into the csm-lab monorepo as `@csm-lab/cl-mock`. Build moves from
`tsc` to tsdown (ESM, bundled), entrypoints split into a library export (`.`) and the
`csm-cl-mock` bin. The binary name is unchanged, so `npx @csm-lab/cl-mock` / `csm-cl-mock`
